...
func (c *FakeContainer) PullImage(ctx context.Context, creds oci.Credentials, useOCIFetcher bool) error {
...
func TestPullImageIfNecessary_ValidCredentials(t *testing.T) {
	...
	err := container.PullImageIfNecessary(ctx, env, c, goodCreds1, false /*=useOCIFetcher*/, imageRef)
	...
	err = container.PullImageIfNecessary(ctx, env, c, goodCreds2, false /*=useOCIFetcher*/, imageRef)
...
func TestPullImageIfNecessary_InvalidCredentials_PermissionDenied(t *testing.T) {
	...
	err := container.PullImageIfNecessary(ctx, env, c, badCreds, false /*=useOCIFetcher*/, imageRef)
	...
	err = container.PullImageIfNecessary(ctx, env, c, goodCreds, false /*=useOCIFetcher*/, imageRef)
...
func TestPullImageIfNecessary_ParallelCallsSerialized(t *testing.T) {
	...
	eg := errgroup.Group{}
	for i := 0; i < 20; i++ {
		eg.Go(func() error { return container.PullImageIfNecessary(ctx, env, c, oci.Credentials{}, false /*=useOCIFetcher*/, imageRef) })
	}
